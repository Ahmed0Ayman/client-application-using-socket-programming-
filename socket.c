...
#define PORT 80 /* HTTP has port 80 as standard port */
...
int main(int argc , char const * argv[]) 
{
	...
	if(argc < 2) /* check if we received valid arguments from terminal */
	{
	...
	char command[100] = "firefox ";

	int socket_id =0 ;
	struct sockaddr_in server_addr ;
	
	/* array of strings used to handle TX & RX buffers */
  	char  mesg[2][BUFFER_SIZE] = {" ","GET / HTTP/1.1\r\n\r\n"};
	...
	/* send required file to read to server */
    	send(socket_id,mesg[1],strlen(mesg[1]),0);
	while(1)
	{
